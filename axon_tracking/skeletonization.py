...
def full_skeletonization(root_path, stream_id, template_id, params, skel_params, qc_params):
    template, template_save_file, noise = load_template_file(root_path, stream_id, template_id)
    if np.mean(noise) > params['max_noise_level']:
        return [], []
    
    temp_diff = np.diff(template)
    capped_template, target_coor= localize_neurons(temp_diff, ms_cutout=params['ms_cutout'])
    ...
    skels = skeletonize(th_template[:,:,t_cap[0]:t_cap[1]].astype('bool'), **skel_params)
    ...
    scaled_qc_list, r2s, vels, lengths = perform_path_qc(all_branches, params,**qc_params)
    
    qc_skel_list = [cv.Skeleton.from_path(x) for x in scaled_qc_list]
    qc_skeleton = cv.Skeleton.simple_merge(qc_skel_list)
    qc_skeleton = kimimaro.postprocess(qc_skeleton, dust_threshold=10, tick_threshold=10)
    ...
    scaled_qc_list, r2s, full_vels, lengths = perform_path_qc(all_branches, params,**qc_params)
...
def localize_neurons(input_mat, ms_cutout, min_distance=5, threshold_rel=0.1, num_peaks=3, buffer_frames=2, ms_peak_cutout = 0.5):
...
def threshold_template(template, noise, target_coor, params):
    if params['noise_threshold']:
        noise_th = template < (params['noise_threshold'] * noise[:,:,:template.shape[2]])
    else:
...
def cone(matrix_shape, r, apex=[]):
    if not apex:
        apex = np.empty(3)
        apex[0], apex[1], apex[2] = np.floor(matrix_shape[0]/2), np.floor(matrix_shape[1]/2), 0
        
    x, y, z = np.ogrid[:matrix_shape[0], :matrix_shape[1], :matrix_shape[2]]
    cone_equation = (x - apex[0])**2 + (y - apex[1])**2 <= r**2 * (1 - (z - apex[2])/matrix_shape[2])**2
...
def generate_dilation_structure(max_t, max_r, spacing=1/3, sampling_rate=20000):
    ...
    structure_base = np.full((structure.shape[0],structure.shape[1],structure.shape[2]),False)
...
def skeletonize(input_matrix, scale=2, const=50, pdrf_exponent=4, pdrf_scale=10000, dust_threshold=0, anisotropy=(17.5,17.5,50.0), tick_threshold=10, n_jobs=16,ais=[]):
...
def perform_path_qc(paths, params, window_size=7, max_duplicate_ratio=0.5, min_r2=0.9, vel_range = [0.4, 1], min_length=10):
    ...
    for path in scaled_paths:
        if len(path) > window_size:
            path, inflection_points = split_path(path,window_size=window_size,max_duplicate_ratio=max_duplicate_ratio)
    ...
    good_vel = np.where((np.array(r2s)>min_r2) & (np.array(vels)<vel_range[1]) & (np.array(vels)>vel_range[0]))
...
def split_path(path, window_size = 7, max_duplicate_ratio=0.3):
    smoothed_data = np.convolve(path[:,2], np.ones(window_size)/window_size,mode='valid')
    ...
    inflection_points = np.delete(inflection_points, [np.where(np.abs(np.diff(inflection_points)) < 2)])
    split_points = [0] + list(inflection_points) + [path.shape[0]]
    #print(split_points)
    split_paths = [path[split_points[s]:split_points[s+1],:] for s in range(len(split_points)-1)]
    qc_paths = remove_circulating_paths(split_paths, max_duplicate_ratio=max_duplicate_ratio)
...
def branches_from_paths(skeleton):
    ...
    for path in path_list:
        path_idx, _ = np.where((path[:, None] == skeleton.vertices[branch_points]).all(axis=2))
        branch_idx = []
        for p in path_idx:
        ...
        if len(branch_idx) > 0:
            ...
            new_branches = [path[branch_idx[x]:branch_idx[x+1],:] for x in range(len(branch_idx)-1)]
...
def remove_circulating_paths(path_list, max_duplicate_ratio=0.3):
    ...
    clean_path_list = [path_list[i] for i in range(len(path_list)) if i not in indices_to_remove]
