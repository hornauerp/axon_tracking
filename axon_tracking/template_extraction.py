...
def extract_templates_from_sorting_dict(sorting_dict, qc_params={}, te_params={}):
    ...
    for rec_path in rec_list:
        ...
        for sorting_path in sorting_list:
            ...
            multirecording, common_el, pos = ss.concatenate_recording_slices(
                rec_path, stream_id
            )
            ...
            extract_all_templates(
                stream_id, segment_sorting, sorting_path, pos, te_params
            )
...
def extract_templates_from_sorting_list(sorting_list, qc_params={}, te_params={}):
    ...
    for sorting_path in tqdm(sorting_list):
        try:
            ...
            segment_sorting = si.SplitSegmentSorting(cleaned_sorting, multirecording)
            extract_all_templates(
                stream_id, segment_sorting, output_path, pos, te_params
            )
...
def extract_templates_from_concatenated_recording(
    root_path, stream_id, qc_params={}, te_params={}
):
    ...
    ax_recording, common_el, pos = ss.concatenate_recording_slices(
        ax_rec_path, stream_id, center=False
    )
    ...
    nw_recording = (
        concat_rec._kwargs["recording_list"][1]
        ._kwargs["recording"]
        ._kwargs["parent_recording"]
    )
...
def extract_waveforms(
    segment_sorting, stream_id, save_root, n_jobs, overwrite_wf, cutout, filter_flag
):
    ...
    for sel_idx, rec_name in enumerate(rec_names):
        ...
        if not os.path.exists(wf_path) or overwrite_wf:
            ...
            seg_we = si.WaveformExtractor.create(
                rec_centered,
                seg_sort,
                wf_path,
...
def align_waveforms(
    ...
    peak_cutout = range(
        np.int16(peak_idx - sample_peak_cutout), np.int16(peak_idx + sample_peak_cutout)
    )
    wfs = seg_we.get_waveforms(sel_unit_id)
    interp_wfs = sp.interpolate.pchip_interpolate(
        list(range(wfs.shape[1])),
        wfs,
        np.linspace(0, wfs.shape[1], num=wfs.shape[1] * upsample),
        axis=1,
    )
    ...
    if align_cutout:
        peak_el = [
            np.where(
                interp_wfs[w, peak_cutout, :]
                == np.nanmin(interp_wfs[w, peak_cutout, :])
            )[1][0]
        ...
        peak_shift = [
            np.where(
                interp_wfs[w, peak_cutout, ref_el]
                == np.nanmin(interp_wfs[w, peak_cutout, ref_el])
            )[0][0]
            for w in range(interp_wfs.shape[0])
        ]
        aligned_length = interp_wfs.shape[1] - 2 * sample_peak_cutout
        aligned_wfs = np.full(
            [interp_wfs.shape[0], np.int16(aligned_length), interp_wfs.shape[2]], np.nan
        )
        for w in range(interp_wfs.shape[0]):
            aligned_wfs[w, :, :] = interp_wfs[
                w, peak_shift[w] : np.int16(peak_shift[w] + aligned_length), :
            ]
    else:
...
def combine_templates(
    ...
    if align_cutout:
        wf_length = np.int16(
            (sum(cutout_samples) - 2 * peak_cutout) * upsample
        )  # length of waveforms after adjusting for potential peak alignments
    ...
    for sel_idx, rec_name in enumerate(rec_names):
        ...
        seg_we = si.load_waveforms(
            os.path.join(save_root, "waveforms", "seg" + str(sel_idx)), sorting=seg_sort
        )
        aligned_wfs = align_waveforms(
            seg_we,
            sel_unit_id,
            cutout_samples,
            peak_cutout,
...
def extract_all_templates(stream_id, segment_sorting, save_root, pos, te_params):
    ...
    extract_waveforms(
        ...
        save_root,
        te_params["n_jobs"],
        te_params["overwrite_wf"],
        cutout_ms,
        te_params["filter_flag"],
    )
    ...
    for sel_unit_id in tqdm(sel_unit_ids):
        ...
        if not os.path.isfile(template_save_file) or te_params["overwrite_tmp"]:
            try:
                template_matrix = combine_templates(
                    stream_id, segment_sorting, sel_unit_id, save_root, **te_params
                )
