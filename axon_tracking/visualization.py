...
def plot_delay_skeleton(path_list, params, skel_params,figsize=4, plot_ais=True, plot_ais_connection=True, linewidth=2,font_size=24):
    ...
    if plot_ais_connection:
        plt.plot([skel_params['ais'][0][0], sorted_vertices[0][0]], [skel_params['ais'][0][1], sorted_vertices[0][1]])
...
def plot_template_overview(root_path, stream_id, params, n_cols = 3, vmin=-10,vmax=0, filename='overview', unit_ids=[], overwrite=False):
...
def plot_skeleton(skeleton, x_lim=[], y_lim=[], fig_size=5, marker_size=10,ais=[]):
    ...
    x_scaling = np.abs(((np.max(x) - np.min(x)) / (np.max(y) - np.min(y))) * 1.15)#[0]
    ...
    clb = plt.colorbar(ticks=[0, max(z)],format=mticker.FixedFormatter(clb_ticks),shrink= 0.5)
